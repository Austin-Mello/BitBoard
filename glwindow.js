...
class GLWindow {
	...
	setTexture(img) {
		...
		if (this.#canv.width > this.#canv.height) {
			this.#zoom = this.#canv.width / this.#texScale.x;
		} else {
			this.#zoom = this.#canv.height / this.#texScale.y;
		}
	...
	updateViewScale() {
		let w = this.#canv.clientWidth;
		let h = this.#canv.clientHeight;
		this.#canv.width = w;
		this.#canv.height = h;
	...
	click(pos) {
		pos.x /= this.#canv.width;
		pos.y /= this.#canv.height;

		let a = {
			x: ((-0.5 * this.#texScale.x - this.#camPos.x) * this.#zoom) / this.#canv.width + 0.5,
			y: ((-0.5 * this.#texScale.y - this.#camPos.y) * this.#zoom) / this.#canv.height + 0.5,
		};
